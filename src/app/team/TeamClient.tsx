...
import { Shield, Award, Users, Clock, Mail, MessageSquare } from 'lucide-react'
import { useLang } from '@/contexts/LanguageContext'

export default function TeamClient() {
  const { lang, isAr } = useLang()

  const teamDepartments = isAr ? [
    {
      icon: <Users size={32} />,
      title: 'السائقون المحترفون',
      desc: 'سائقونا مدربون على كرم الضيافة وخدمة ضيوف الرحمن، ويتحدثون العربية والإنجليزية بطلاقة. لديهم معرفة شاملة بالطرق والمشاعر المقدسة وأوقات الذروة في مكة والمدينة وجدة والطائف.',
    },
    {
      icon: <Clock size={32} />,
      title: 'إدارة العمليات والتنسيق',
      desc: 'فريق التنسيق يعمل على مدار 24 ساعة لمتابعة الحجوزات وتنظيم الاستقبال في المطارات ونقل الحجاج والمعتمرين لضمان دقة المواعيد وسلاسة الرحلات.',
    },
    {
      icon: <Shield size={32} />,
      title: 'خدمة العملاء والدعم',
      desc: 'فريق دعم ثنائي اللغة متواجد على الواتساب للرد السريع على الاستفسارات، تأكيد الحجوزات، ومتابعة تجربة العملاء خطوة بخطوة.',
    },
    {
      icon: <Award size={32} />,
      title: 'صيانة وتجهيز الأسطول',
      desc: 'فريق فني متخصص يحافظ على نظافة وجاهزية المركبات (تويوتا كامري، هيونداي ستاريا، وجي إم سي يوكن) مع إجراء فحوصات دورية شاملة.',
    },
  ] : [
    {
      icon: <Users size={32} />,
  ...
  const values = isAr ? [
    { title: 'الأمان أولاً', desc: 'سائقون ذوو خبرة ومركبات تخضع للفحص الدوري الشامل لضمان سلامتك التامة.' },
    { title: 'أسعار ثابتة ومحددة', desc: 'بدون عدادات أو رسوم مفاجئة أو تسعير وقت الذروة. السعر المتفق عليه هو ما تدفعه.' },
    { title: 'خدمة ضيوف الرحمن', desc: 'خبرة متخصصة بمسارات الحج والعمرة، التوقف للصلوات والميقات، وأعلى معايير الضيافة.' },
    { title: 'أسطول فاخر وحديث', desc: 'مركبات مريحة ونظيفة تشمل تويوتا كامري، هيونداي ستاريا، وجي إم سي يوكن VIP.' },
  ] : [
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--background)', paddingTop: '80px' }} dir={isAr ? 'rtl' : 'ltr'}>
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #071f17, #0B3D2E)', color: 'white', padding: '64px 0 48px', textAlign: 'center' }}>
        <div className="container" style={{ maxWidth: '760px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: '900', margin: '16px 0', lineHeight: 1.2 }}>
            {isAr ? (
              <>تعرف على فريق <span style={{ color: 'var(--primary)' }}>سعودي كاب جي إم سي</span></>
            ) : (
          ...
          <p style={{ opacity: 0.85, lineHeight: 1.8, fontSize: '1.05rem' }}>
            {isAr 
              ? 'فريق متخصص ومتفانٍ يعمل على مدار الساعة لجعل كل رحلة في المملكة العربية السعودية آمنة، مريحة، ولا تُنسى.'
      ...
      <section style={{ padding: '48px 0', backgroundColor: 'var(--muted)', borderTop: '1px solid var(--border)' }}>
        <div className="container" style={{ maxWidth: '760px' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: '900', marginBottom: '24px', textAlign: 'center' }}>
            {isAr ? (
              <>قيمنا <span style={{ color: 'var(--primary)' }}>الأساسية</span></>
            ) : (
      ...
      <section style={{ padding: '60px 0', background: 'linear-gradient(135deg, #071f17, #0B3D2E)', color: 'white', textAlign: 'center' }}>
        <div className="container" style={{ maxWidth: '560px' }}>
          <h2 style={{ fontSize: '1.6rem', fontWeight: '900', marginBottom: '12px' }}>
            {isAr ? 'هل ترغب بالانضمام إلى فريقنا؟' : 'Want to Join Our Team?'}
          </h2>
          <p style={{ opacity: 0.85, marginBottom: '28px', lineHeight: 1.7 }}>
            {isAr
              ? 'نرحب دائماً بالسائقين المحترفين ومنسقي خدمات النقل المتميزين في مختلف مدن المملكة.'
          ...
          <div style={{ display: 'flex', gap: '14px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a
              ...
              style={{
                ...
                padding: '14px 28px', borderRadius: '12px', textDecoration: 'none', fontSize: '1rem',
              }}
            >
              <Mail size={18} />
              {isAr ? 'التقديم عبر البريد الإلكتروني' : 'Apply via Email'}
            </a>
            <a
              href="https://wa.me/966569489449?text=Hello%2C%20I%20am%20interested%20in%20joining%20the%20Saudi%20Cabs%20GMC%20team"
              target="_blank"
